...
class Net(nn.Module):
    ...
    def forward(self, data: Dict) -> Dict[str, List[Tensor]]:
        ...
        nodes_seq_1 = self.a2m(nodes, graph, actors, actor_idcs, actor_ctrs)
        nodes_seq_2 = self.m2m(nodes_seq_1, graph)

        actors_seq_1 = self.m2a(actors, actor_idcs, actor_ctrs, nodes, node_idcs, node_ctrs)
        actors_seq_2 = self.a2a(actors_seq_1, actors, actor_idcs, actor_ctrs)

        nodes_actors = self.m2a(actors_seq_2, actor_idcs, actor_ctrs, nodes_seq_2, node_idcs, node_ctrs)

        # prediction
        out = self.pred_net(nodes_actors, actor_idcs, actor_ctrs)
...
class A2M(nn.Module):
    ...
    def __init__(self, config):
        ...
        att = []
        for i in range(2):
            att.append(MultiHeadAttention(n_map, config["n_actor"]))
...
class M2A(nn.Module):
    ...
    def __init__(self, config):
        ...
        att = []
        for i in range(2):
            att.append(MultiHeadAttention(n_actor, n_map))
        self.att = nn.ModuleList(att)
...
class A2A(nn.Module):
    ...
    def __init__(self, config):
        ...
        att = []
        for i in range(2):
            att.append(MultiHeadAttention(n_actor, n_actor))
        self.att = nn.ModuleList(att)

    def forward(self, actors: Tensor, ctxs: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor]) -> Tensor:
        for i in range(len(self.att)):
            actors = self.att[i](
                actors,
                actor_idcs,
...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_agt: int, n_ctx: int) -> None:
        ...
        self.num_heads = math.gcd(n_agt, 8)
        self.att_head_size = n_agt // self.num_heads
        ...
        self.query = nn.Linear(n_agt, n_agt)
        self.key = nn.Linear(n_ctx, n_agt)
        self.value = nn.Linear(n_ctx, n_agt)

        self.ctx = nn.Sequential(
            Linear(2 * n_agt, n_agt, norm=norm, ng=ng),
            nn.Linear(n_agt, n_agt, bias=False),
        )
    ...
    def forward(self, agts: Tensor, agt_idcs: List[Tensor], 
                agt_ctrs: List[Tensor], ctx: Tensor, ctx_idcs: List[Tensor], 
                ctx_ctrs: List[Tensor], dist_th: float) -> Tensor:
        res = agts
        if len(ctx) == 0:
            agts = self.agt(agts)
            agts = self.relu(agts)
        ...
        query = self.query(agts[hi]).view(-1, self.num_heads, self.att_head_size)
        key = self.key(ctx[wi]).view(-1, self.num_heads, self.att_head_size)
        value = self.value(ctx[wi]).view(-1, self.num_heads, self.att_head_size)

        attention = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(self.att_head_size)
        attention = torch.softmax(attention, dim=-1)
        attention = torch.matmul(attention, value)
        attention = attention.view(-1, self.num_heads * self.att_head_size)

        ctx = torch.cat((dist, attention), 1)
        ctx = self.ctx(ctx)
